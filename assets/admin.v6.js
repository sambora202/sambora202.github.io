...
function renderTrackEditor(section, track, index) {
  ...
  return `<article class="track-editor" data-section="${section}" data-index="${index}">
...
function renderTracks(section) {
  ...
  form.innerHTML = `
    ...
    ${(data.tracks || []).map((track, index) => renderTrackEditor(section, track, index)).join('') || `<div class="status warn">No ${section} items yet. Click Add ${section === 'music' ? 'music item' : 'sketch item'}.</div>`}
  `;
}
